...
Every outgoing op should also carry `schemaVersion` (see `SCHEMA_VERSION` in `index.js`). The server only validates its range when present and doesn't reject a missing one outright, but it's a required, non-nullable column in the real `operations` table (`prisma/schema.prisma` in the real app) — every op the real client writes carries it, so ours should too.

### `res.latestSeq` is the account's high-water mark, not a pagination cursor

Both `downloadOps` and `uploadOps` responses include a `latestSeq` field. It is **not** "how far this request got" — it's `userSyncState.lastSeq`, the account's overall high-water mark, confirmed against the real server's `operation-download.service.ts` (`const latestSeq = seqRow?.lastSeq ?? 0`). Using it as the next page's `sinceSeq` (as `pullPage()` used to) skips straight to the account's current end on page 1 whenever `hasMore` is true, silently losing every op in between forever — the skipped range is never revisited since `lastSeq` gets persisted at the inflated value. The correct resume point is the highest `serverSeq` actually seen in the current page's `ops` array. The same misuse after a single-op upload (in `uploadSingleOp`) corrupted the cursor after every toggle/track-time upload, not just multi-page downloads — fixed by not saving `res.latestSeq` there at all; `lastSeq` now only advances via `pullPage()`'s own per-page tracking, and the default-on `runAutoSyncAfterOp()` follow-up sync picks up anything the upload response piggybacked. Verified live against a real ~3,600-op account (`sinceSeq` correctly walked forward per page instead of jumping to the end on page 1).

### AppMessage send failures used to be completely silent

`outbox_failed_handler` in `main.c` only logged to `APP_LOG` — if a watch→phone send (task toggle, track-time-stop, resync request) failed for any reason (phone app backgrounded, Bluetooth momentarily busy), nothing appeared on screen; the phone-side JS that would normally report a sync error never even received anything to react to. This is now routed through the same fullscreen error overlay as a phone→watch sync error. Similarly, `STATUS_NOT_PAIRED` used to only render on the empty/first-run screen (`update_empty_layer`) — invisible once a cached task list was already showing, since the Resync row's status switch didn't have a case for it. Both were real, confirmed gaps, found while investigating a report of watch-tracked time never reaching the desktop app with no visible error — a live, fully-paginated fetch of the real account (see the `res.latestSeq` note above) turned up **zero** ops from any `pebble-*` clientId anywhere in retained history, meaning no watch-originated upload had ever been confirmed stored server-side, consistent with a delivery failure this silent.
...
### Time tracking is a delta, not a snapshot

Long-select on a task starts/stops tracking it (`main.c`'s `menu_select_long_click`/`start_tracking`/`stop_tracking_and_report`). Confirmed against the real `syncTimeSpent` action (`time-tracking.actions.ts`) and its reducer (`task.reducer.ts`): the op payload is `{ taskId, date, duration }` where `duration` is a **delta in milliseconds for that calendar day** — never a cumulative total, never the full `timeSpentOnDay` map. The real reducer applies it additively (`timeSpentOnDay[date] = (existing || 0) + duration`) so concurrent contributions from other clients merge instead of clobbering; `task-store.js`'s replay does the same. Only one task tracks at a time, globally (mirrors the real app's single `currentTaskId`) — starting a long-select on a different task stops-and-flushes whichever one was running first. The watch persists the tracking session (task id + start epoch, via `Storage`) across an app close/relaunch rather than tying it to the window's lifetime; elapsed time is always derived from that persisted start timestamp, not accumulated tick-by-tick, so nothing is lost if the watchapp closes mid-session.

### Component map

- **`src/c/main.c`** — the watchapp UI. A sectioned `MenuLayer`: a red "Resync" row pinned to section 0 (status/subtitle reflects live sync state), then either one section per project group or a single unheaded section. Every task row shares one custom draw path (`menu_draw_row`, not `menu_cell_basic_draw`) so title size/background/selection-color are consistent, since Pebble's default cell drawing can't do a centered "Done" subtitle or match custom colors. A too-wide selected title marquee-scrolls (`AppTimer`, 300ms tick); a too-long title is otherwise ellipsis-truncated to one line, never wrapped (the title's text box height is deliberately constrained to one measured line). A sync error takes over the whole content area (red, word-wrapped, dismissed by Select) rather than a truncated subtitle — see `show_error_overlay`/`hide_error_overlay`. Persists the last-synced list and any in-progress time-tracking session via the `Storage` API.
- **`src/pkjs/index.js`** — the sync engine: `doSync()` (download+replay+push-to-watch), `handleTaskToggle()`/`handleTrackTimeStop()` (upload a watch-side change; both funnel through the shared `uploadSingleOp()`/`runAutoSyncAfterOp()` helpers for consistent accept/reject handling), pairing/config persistence (`localStorage`: `sp_config`, `sp_password`, `sp_entities`, `sp_last_seq`, `sp_vector_clock`, `sp_client_id`).
...
| phone→watch | `TASK_ITEM` | one task | `TASK_INDEX`, `TASK_ID`, `TASK_TITLE`, `TASK_DONE`, `TASK_PROJECT`, `TASK_DUE_MIN` (minutes since local midnight; omitted when the task has no `dueWithTime`), `TASK_TIME_SPENT_MS` (omitted when there's no tracked time) |
